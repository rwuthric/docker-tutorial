...
In this example we build a docker image for running a simple python script which prints "Hello world" to the screen.

## Creating the Dockerfile
The `Dockerfile` contains all build instructions for the image. In our case we build our image based on the [official Python image](https://hub.docker.com/_/python) on docker hub.
...
```
With the command `RUN`, we tell docker to run a command inside our image. In this case we run the linux `useradd` command (note that the python docker image comes as a Debian Linux distribution) to create a new user `test_user`. With the option `--create-home` we ask to not only create this new user, but as well to create the home directory of this user.

Next we switch our user from the root user to this new user
```
USER test_user
```
and then change our working directory to the home directory of the `test_user`
```
...
COPY hello.py .
```
Note the paths. For the source path (first argument of the `COPY` command), the path is relative to the location of the `Dockerfile`. For the destination path, it is relative to the working directory in our docker image. We could as well have used `COPY hello.py /home/test_user` instead if we want to put an absolute path.
...
You can check that your image is now built by inspecting the images present in your docker:
```
docker images
```
...
```
We find indeed our container `ex1`. Its status is `Exited (0)` which tels that the container exited and the rrro code was `0` (i.e. no error).
...
/home/test_user
```
This interactive mode is useful to look around within the container and very handy for debugging when developing the `Dockerfile`.
